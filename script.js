...
// =========================
// CERTIFICATIONS — DATA-DRIVEN LIST
// =========================
// Add new certificates here — the list, the "show more" toggle,
// and the layout all update automatically. No HTML editing needed.
const CERTS = [
  { name: "Cambridge International A-Level", issuer: "Cambridge Assessment International Education", year: "2025" },
  { name: "Pearson Edexcel International A-Level", issuer: "Pearson Edexcel", year: "2025" },
];

const CERT_VISIBLE_COUNT = 3; // how many show before "Show more" is needed

const CERT_ICON_SVG = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="6"/><path d="M15.477 12.89L17 22l-5-3-5 3 1.523-9.11"/></svg>`;

const certList = document.getElementById("certList");
const certToggle = document.getElementById("certToggle");
const certToggleText = document.getElementById("certToggleText");

function buildCertItem(cert, hidden) {
  const li = document.createElement("li");
  li.className = "cert-item" + (hidden ? " cert-hidden" : "");
  li.innerHTML = `
    <div class="cert-icon">${CERT_ICON_SVG}</div>
    <div>
...
if (certList) {
  CERTS.forEach((cert, i) => {
    certList.appendChild(buildCertItem(cert, i >= CERT_VISIBLE_COUNT));
  });

  const hiddenCount = Math.max(0, CERTS.length - CERT_VISIBLE_COUNT);

  if (hiddenCount > 0 && certToggle) {
    certToggle.style.display = "flex";
    certToggleText.textContent = `Show ${hiddenCount} more`;

    let expanded = false;
    certToggle.addEventListener("click", () => {
      expanded = !expanded;
      certList.querySelectorAll(".cert-hidden").forEach(cert => {
        cert.classList.toggle("cert-visible", expanded);
      });
      certToggleText.textContent = expanded ? "Show less" : `Show ${hiddenCount} more`;
